...
const MasterclassSalesTeam = () => {
  ...
  const loadRoles = async () => {
    try {
      ...
      setRoles((data || []).filter(r => r.role === 'masterclass_sales'));
    } catch (err: any) {
  ...
  const handleAddRole = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewRole({ input: '', type: 'email', role: 'masterclass_sales' });
    } catch (err: any) {
  ...
  const handleDeleteRole = async (userId: string) => {
    if (!window.confirm('Are you sure you want to remove this sales user?')) return;
  ...
  return (
    <AdminLayout title="Masterclass Sales Team">
      <div className="p-6">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-black text-gray-900 tracking-tight">Sales Team</h1>
            <p className="text-gray-500 text-sm">Manage sales users for Masterclass</p>
          </div>
          <button
              ...
              {loading ? (
                ...
                roles.map((role) => (
                  <tr key={role.user_id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border bg-indigo-100 text-indigo-800 border-indigo-200">
                        {role.role.replace('_', ' ')}
                      </span>
                    </td>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[2rem] max-w-md w-full p-8 shadow-2xl">
            <h2 className="text-2xl font-black text-gray-900 mb-2">Assign Sales Role</h2>
            <p className="text-gray-500 text-sm mb-6">Assign sales permissions to a registered user.</p>
            ...
            <form onSubmit={handleAddRole} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  required
                  placeholder={newRole.type === 'email' ? 'e.g. sales@yenege.com' : 'e.g. 550e8400-e29b-41d4-a716-...'}
                  value={newRole.input}
...
export default MasterclassSalesTeam;
