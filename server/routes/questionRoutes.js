...
router.post('/', authenticateUser, requireAdmin, logAdminActivity('CREATED_QUESTION', null, (req) => ({ gameTypeId: req.body.gameTypeId, questionText: req.body.questionText })), async (req, res) => {
  try {
    const { gameTypeId, questionText, options, correctAnswer, difficulty } = req.body;
...
router.put('/:id', authenticateUser, requireAdmin, logAdminActivity('UPDATED_QUESTION', (req) => req.params.id, (req) => ({ gameTypeId: req.body.gameTypeId, questionText: req.body.questionText })), async (req, res) => {
  try {
    const { id } = req.params;
    const { gameTypeId, questionText, options, correctAnswer, difficulty } = req.body;
    const { rows } = await pool.query(
...
router.delete('/:id', authenticateUser, requirePermission('can_manage_questions'), requestPermission('DELETE_QUESTION', 'questions', (req) => req.params.id, (req) => ({ questionId: req.params.id })), async (req, res) => {
  try {
    const { id } = req.params;
