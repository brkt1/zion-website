...
async function runGameHistoryMigration() {
  try {
    console.log('Running game history migration...');
    
    // Read the migration SQL file
    const migrationSQL = readFileSync(join(__dirname, 'db', 'user_game_history.sql'), 'utf8');
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      if (statement.trim()) {
        ...
        if (error) {
          ...
          const { error: directError } = await supabase.from('user_game_history').select('id').limit(1);
          
          if (directError && directError.code === '42P01') {
            // Table doesn't exist, we need to create it manually
            console.log('Creating user_game_history table manually...');
            ...
            console.log('\nAnd the rest of the migration from db/user_game_history.sql');
            break;
          }
        }
      }
    ...
    if (error) {
      ...
      console.log('Migration verified - user_game_history table exists!');
    }
